...
class MapperBigData1 extends Mapper<
    ...
    protected void map(
    		...
    		String userID = Integer.parseInt(lineEntries[1]);
    		String itemID = Integer.parseInt(lineEntries[2]);

    		// if timestamp is in ISO format, or a lexically orderable equivalent
    		// (which it is)
    		// I can just use the string comparison, otherwise I need a parser
    		// can't remind the exact java syntax, here I mean that
    		// if timestamp is before 1/1/2020 OR after 21/12/2023
    		// then we return (i.e., skip thi line) as we dont need it
    		...
    		if (timestamp.compare(startTime) < 0 || timestamp.compare(endTime) > 0)

            // We emit User: Item
            // the reducer will receive an iterable, not a full structured array
            // thus it will be menageable in memory as long as we dont convert it
